...
//password Hashing, login and register functions
...
export const verifyPassword = async (password: string, hash: string) => {
 ...
 export const LogInUser = async (email: string, password: string) => {
	...
	const passwordMatch = await verifyPassword(password, FindUser.passwordHash);
		if(!passwordMatch){
			return{
				user:null,
	    ...
	    const session = await createSession(token,FindUser.id);
